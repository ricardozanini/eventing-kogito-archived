...
func (r *KogitoRuntimeReconciler) ReconcileKogitoRuntime(
	...
	if apierrors.IsNotFound(err) {
		syncSink(ctx, binder, &expected.Spec)
	...
	if kogitoRuntimeSpecSync(ctx, binder, expected.Spec, &ra.Spec) {
...
func kogitoRuntimeSpecSync(ctx context.Context, binder *sourcesv1.SinkBinding, expected v1beta1.KogitoRuntimeSpec, now *v1beta1.KogitoRuntimeSpec) bool {
	old := *now.DeepCopy()
	syncLabels(expected, now)
	syncSink(ctx, binder, now)
	return !equality.Semantic.DeepEqual(old, now)
}

func syncLabels(expected v1beta1.KogitoRuntimeSpec, now *v1beta1.KogitoRuntimeSpec) {
	for labelKey := range expected.DeploymentLabels {
		now.DeploymentLabels[labelKey] = expected.DeploymentLabels[labelKey]
		now.ServiceLabels[labelKey] = expected.ServiceLabels[labelKey]
	}
}

func syncSink(ctx context.Context, binder *sourcesv1.SinkBinding, now *v1beta1.KogitoRuntimeSpec) {
	...
	ps.Spec.Template.Spec.Containers[0].Env = now.Env
	binder.Do(ctx, ps)
	...
	now.Env = ps.Spec.Template.Spec.Containers[0].Env
}
